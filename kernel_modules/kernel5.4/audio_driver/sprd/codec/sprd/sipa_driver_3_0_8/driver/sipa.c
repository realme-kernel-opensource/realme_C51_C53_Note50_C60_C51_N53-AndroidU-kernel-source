...
static sipa_dev_t *g_default_sipa_dev = NULL;
...
int sipa_audio_power_scene_set(int on)
{
    ...
    pr_debug("[debug][%s] %s: on_off = %d \r\n",
		LOG_FLAG, __func__, on);
		
		if (likely(NULL != g_default_sipa_dev )) {
			si_pa = g_default_sipa_dev ;
		} else {
			pr_err("[  err][%s] %s: g_default_sia_dev == NULL !!! \r\n",
				LOG_FLAG, __func__);
			return EINVAL;
...
EXPORT_SYMBOL(sipa_audio_power_scene_set);
	...
	struct device_node	*sipa_of_node)
{
	...
	if (0 != ret) {
		...
		if (NULL == si_pa) {
			...
			strncpy(si_pa->name, si_pa_dev_name, strlen(si_pa_dev_name));
		}
	}
