...
static int npi_nl_handler(struct sk_buff *skb_2, struct genl_info *info)
{
	...
	struct sprd_npi_cmd_hdr *hdr = NULL;
        unsigned short r_len = SPRD_NPI_RECV_BUF_LEN, s_len = 0;
...
void sprd_npi_set_cca_param(struct sprd_priv *priv, struct sprd_vif *vif, u8 flag)
{
	struct sprd_npi_cmd_hdr *hdr = NULL;
	unsigned short r_len = SPRD_NPI_RECV_BUF_LEN, s_len = 0;
	...
	/* flag: 1 means set adaptive enable, 2 means set acs enable */
	if (flag == 1) {
		spin_lock_bh(&adap_info.adap_lock);
...
{
        sprd_npi_set_cca_param(priv, vif, 1);
}
