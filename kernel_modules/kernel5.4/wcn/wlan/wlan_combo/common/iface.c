...
#ifdef ENABLE_CHR
			hif->chr->open_err_flag = OPEN_ERR_POWER_ON;
#endif
			return ret;
		}
...
static int iface_open(struct net_device *ndev)
{
	...
	if (chr->open_err_flag) {
		if (chr->sock_flag == 1 && chr->drv_cmd_list[0].set)
			sprd_report_chr_open_error(chr, EVT_CHR_OPEN_ERR, chr->open_err_flag - 1);
		else
			pr_info("%s, CHR: open err appears, but chr module is closed\n", __func__);
		chr->open_err_flag = OPEN_ERR_INIT;
	}
#endif
	if (ret)
		return ret;
...
static int iface_close(struct net_device *ndev)
{
	...
	/* CR 2352989, 2333930, 2360958
	 * wdev->current_bss is not NULL when iface_close comes,
	 * call cfg80211_disconnected to clear wdev->current_bss.
	 */
	if (vif->wdev.iftype == NL80211_IFTYPE_STATION ||
		...
		if (vif->sm_state == SPRD_DISCONNECTING ||
			...
			pr_info("%s mode %d sm_state %d for sta or p2p gc.\n",
