...
static const char *cmd2str(u8 cmd)
{
	...
	case WIFI_CMD_SET_CHR:
		return "WIFI_CMD_SET_CHR";
	default:
...
#define SEC4	4
	...
	if (ret) {
		...
		kfree(wifi_data);
		priv->chr->open_err_flag = OPEN_ERR_DOWNLOAD_INI;
		wlan_set_assert(priv, SPRDWL_MODE_NONE,
				WIFI_CMD_DOWNLOAD_INI,
...
int sprdwl_set_chr(struct sprdwl_chr *chr)
{
	struct sprdwl_msg_buf *msg;
	struct sprdwl_cmd_chr_mode *p;
	struct sprdwl_priv *priv = chr->priv;
	struct chr_cmd tcmd = {0};
	int left = 0;
	int index = 0;

	msg = sprdwl_cmd_getbuf(priv, sizeof(*p), SPRDWL_MODE_NONE,
				SPRDWL_HEAD_RSP, WIFI_CMD_SET_CHR);
	if (!msg)
		return -ENOMEM;

	p = (struct sprdwl_cmd_chr_mode *)msg->data;
...
void sprdwl_event_chr(struct sprdwl_vif *vif, u8 *data, u16 len)
{
	...
	case EVT_CHR_DISC_SYS_ERR:
		sprdwl_report_chr_disconnection(vif, echr.version, echr.evt_id,
