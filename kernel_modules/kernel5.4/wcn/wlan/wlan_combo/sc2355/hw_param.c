...
int sc2355_get_nvm_table(struct sprd_priv *priv, struct wifi_conf_t *p)
{
	...
	if (prj_name !=NULL && (strncmp(prj_name,"22716",5) ==0 )) {
		...
		return hw_param_nvm_parse(priv, SYSTEM_WIFI_CONFIG_FILE_HULKBC, (void *)p);
	}else if(prj_name !=NULL && (strncmp(prj_name,"23633",5) ==0 )){
		printk("[kernel] SYSTEM_WIFI_CONFIG_FILE_HULKBC 23633");
		return hw_param_nvm_parse(priv, SYSTEM_WIFI_CONFIG_FILE_HULKBC, (void *)p);
	}else if(prj_name !=NULL && (strncmp(prj_name,"23711",5) ==0 )){
		printk("[kernel] SYSTEM_WIFI_CONFIG_FILE_HULK 23711");
		return hw_param_nvm_parse(priv, SYSTEM_WIFI_CONFIG_FILE_HULK, (void *)p);
	}else if(prj_name !=NULL && (strncmp(prj_name,"23712",5) ==0 )){
		printk("[kernel] SYSTEM_WIFI_CONFIG_FILE_HULK 23712");
		return hw_param_nvm_parse(priv, SYSTEM_WIFI_CONFIG_FILE_HULK, (void *)p);
	}else if(prj_name !=NULL && (strncmp(prj_name,"23713",5) ==0 )){
		printk("[kernel] SYSTEM_WIFI_CONFIG_FILE_HULK 23713");
		return hw_param_nvm_parse(priv, SYSTEM_WIFI_CONFIG_FILE_HULK, (void *)p);
	}else{
