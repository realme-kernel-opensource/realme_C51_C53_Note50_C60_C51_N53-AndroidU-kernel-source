		...
		struct isp_k_block *isp_k_param, uint32_t idx)
{
	...
	struct dcam_dev_raw_gtm_bypass *gtm_bypass_info = NULL;
	...
	case DCAM_PRO_GTM_CALC_MODE:
        ...
        gtm_bypass_info = &isp_k_param->gtm_rgb_info.bypass_info;
        ret = copy_from_user((void *)gtm_bypass_info, param->property_param, sizeof(struct dcam_dev_raw_gtm_bypass));
        if (ret) {
            pr_err("fail to copy, ret=0x%x\n", (unsigned int)ret);
            return -EPERM;
        }
