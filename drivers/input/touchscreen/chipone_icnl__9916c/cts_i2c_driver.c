...
#define LCD_NAME_LD_ICNL9916_CD "lcd_icnl9916c_hjc_cd_mipi_hd"
...
#endif
{
    ...
    cts_data->cts_dev.rtdata.pm_suspend = false;
    init_completion(&cts_data->cts_dev.rtdata.pm_completion);
...
static int cts_pm_suspend(struct device *dev)
{
    ...
    cts_dev->rtdata.pm_suspend = true;
#if KERNEL_VERSION(3, 12, 0) >= LINUX_VERSION_CODE
    cts_dev->rtdata.pm_completion.done = 0;
#else
    reinit_completion(&(cts_dev->rtdata.pm_completion));
#endif
    return 0;
}
...
static int cts_pm_resume(struct device *dev)
{
    ...
    cts_info("system resume from pm_suspend");
    cts_dev->rtdata.pm_suspend = false;
    complete(&(cts_dev->rtdata.pm_completion));
    return 0;
}
...
static int __init cts_driver_init(void)
{
	...
	if ((!retval) && (!strstr(lcd_name, LCD_NAME_LD_ICNL9916)) && (!strstr(lcd_name, LCD_NAME_LD_ICNL9916_INX)) && (!strstr(lcd_name, LCD_NAME_LD_ICNL9916_CD)))
	{
    ...
    if (strstr(lcd_name, LCD_NAME_LD_ICNL9916_INX)) {
        if(ishulk_10w > 0){
            g_firmware_version_cts = 11;
        }else{
            g_firmware_version_cts = 1;
        }
    } else if (strstr(lcd_name, LCD_NAME_LD_ICNL9916_CD)) {
        g_firmware_version_cts = 2;
    } else if (strstr(lcd_name, LCD_NAME_LD_ICNL9916)) {
        if(ishulk_10w > 0){
            g_firmware_version_cts = 10;
        }else{
            g_firmware_version_cts = 0;
        }
    }
