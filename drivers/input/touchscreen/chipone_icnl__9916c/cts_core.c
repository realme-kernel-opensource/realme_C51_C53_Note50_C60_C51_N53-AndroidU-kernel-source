...
static char firmware_versionK[25] =     {'H','U','L','K','_','K','_','L','D','_','M','D','T','_','N','L','0','4','0','0'};
static char firmware_version_inx[25] =  {'H','U','L','K','_','T','o','k','e','n','_','I','N','X','_','N','L','0','6','0','0'};
static char firmware_version_cd[25] =   {'H','U','L','K','_','L','D','_','M','D','T','_','N','L','0','7','0','0'};
static char firmware_version_inxK[25] = {'H','U','L','K','_','K','_','T','o','k','e','n','_','H','K','C','_','N','L','0','6','0','0'};

/*FOR oplus tp interface*/
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    } else if (g_firmware_version_cts == 2) {
        firmware_version_cd[16] = hex_str[2];
        firmware_version_cd[17] = hex_str[3];
        get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version_cd);
        cts_info("Hardinfo_tp_fw_ver:%s\n",firmware_version_cd);
    } else if (g_firmware_version_cts == 10) {
        firmware_versionK[18] = hex_str[2];
        firmware_versionK[19] = hex_str[3];
        get_hardware_info_data(HWID_CTP_FW_INFO, firmware_versionK);
        cts_info("Hardinfo_tp_fw_ver:%s\n",firmware_versionK);
    } else if (g_firmware_version_cts == 11) {
        firmware_version_inxK[21] = hex_str[2];
        firmware_version_inxK[22] = hex_str[3];
        get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version_inxK);
        cts_info("Hardinfo_tp_fw_ver:%s\n",firmware_version_inxK);
...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if (unlikely(cts_dev->rtdata.suspended)) {
        if (cts_dev->rtdata.gesture_wakeup_enabled) {
            __pm_stay_awake(hulk_tp_data->ws);
            if (cts_dev->rtdata.pm_suspend) {
                /* Waiting for pm resume completed */
                ret = wait_for_completion_timeout(&cts_dev->rtdata.pm_completion, msecs_to_jiffies(700));
                if (!ret) {
