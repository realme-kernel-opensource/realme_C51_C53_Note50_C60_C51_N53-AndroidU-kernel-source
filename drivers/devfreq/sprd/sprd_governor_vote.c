...
static ssize_t ddrinfo_mask_freq_table_show(struct device *dev,
	...
	char *buf)
{
	...
	for (i = 0; i < freq_num; i++) {
		err = gov_callback->get_mask_freq_table(&data, i);
		if (!err)
			count += sprintf(&buf[count], "%u ", data);
	}
	count += sprintf(&buf[count], "\n");

	gov_callback->ddr_dfs_step_add(get_mask_freq_table_t,
				0, NULL, freq_num, task_pid_nr(current));

	return count;
}
static DEVICE_ATTR_RO(ddrinfo_mask_freq_table);

static ssize_t scaling_mask_freq_set_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	...
	err = sscanf(buf, "%u %u\n", &freq, &is_mask);
	if (err < 2) {
		dev_warn(dev->parent, "get mask para err: %d", err);
		return count;
	}

	err = gov_callback->set_mask_freq_table(freq, is_mask);
	if (err)
		dev_err(dev, "mask freq fail: %u %u", freq, is_mask);

	gov_callback->ddr_dfs_step_add(set_mask_freq_t,
				err, NULL, freq, task_pid_nr(current));

	return count;
}
static DEVICE_ATTR_WO(scaling_mask_freq_set);
...
static struct attribute *dev_entries[] = {
	&dev_attr_ddrinfo_mask_freq_table.attr,
	&dev_attr_scaling_mask_freq_set.attr,
...
int ddr_freq_mask(unsigned int target_freq, bool is_mask)
{
	int err;
	struct governor_callback *gov_callback;
	struct devfreq *devfreq = gov_devfreq;
	...
	dev_info(devfreq->dev.parent, "mask freq %d %d\n", target_freq, is_mask);

	err = gov_callback->set_mask_freq_table(target_freq, is_mask);
	if (err)
		dev_err(devfreq->dev.parent, "mask freq %d fail", target_freq);

	return err;
}
EXPORT_SYMBOL(ddr_freq_mask);
