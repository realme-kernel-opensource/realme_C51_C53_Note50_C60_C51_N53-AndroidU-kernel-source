...
static void mmc_swcq_change_ddr(bool is_mask);
...
void sdhci_sprd_timeout_register(void *callback)
{
	struct mmc_swcq *swcq = g_swcq;

	if (swcq != NULL)
		swcq->panic_callback = callback;
	else
		pr_err("g_swcq = NULL\n");
}
EXPORT_SYMBOL(sdhci_sprd_timeout_register);

static void mmc_swcq_change_ddr(bool is_mask)
{
	struct mmc_swcq *swcq = g_swcq;

	if (swcq->panic_callback != NULL) {
		swcq->panic_callback(is_mask);
	} else {
		pr_err("The call_back is NULL\n");
	}
}

static void mmc_swcq_restore_ddr(struct mmc_host *mmc)
{
	struct mmc_swcq *swcq = g_swcq;

	if (swcq->panic_callback != NULL) {
		swcq->panic_callback(false);
	} else {
		pr_err("The call_back is NULL\n");
	}
}
...
int mmc_swcq_init(struct mmc_swcq *swcq, struct mmc_host *mmc)
{
	...
	g_swcq = swcq;
	swcq->panic_callback = NULL;
	swcq->mmc = mmc;
	swcq->mmc->cqe_private = swcq;
	mmc_swcq_ops.android_kabi_reserved1 = (u64)mmc_swcq_is_busy;
	mmc_swcq_ops.android_kabi_reserved2 = (u64)mmc_swcq_change_ddr;
